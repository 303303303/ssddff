...
def main():
    ...
    st.set_page_config(page_title='채용공고 추천 시스템',layout='wide')
    ...
    with tab5:
        ...
        st.divider()
        st.image("주요사업.png")
        st.write("- 응용 소프트웨어 개발: 모빌리티 플랫폼/ 금융 플랫폼/ 식자재 주문 관리 플랫폼/ 에너지 플랫폼/ 구직 및 취업 플랫폼/ 자동화 기계 시스템/ 광고 대행 소프트웨어/ 온라인 게임/ 웹 기반 디자인 툴/ 검색 서비스")
        st.write("- 이커머스: 쇼핑몰/ 오픈마켓/ 소셜 커머스/ 전자상거래/ 통신 판매/ 화장품/ 의류/ 음식 배달대행 서비스")
        st.write("- 마케팅: 모바일 마케팅 서비스/ 디지털 마케팅 솔루션/ 모바일 광고/ 종합 광고/ 홍보/ 서적/ 잡지/ 영화")
        st.write("- 금융: 암호화폐/ P2금융/ 온라인 정보 제공/ 대출 서비스/ 보험 서비스/ 금융 경제 미디어/ 금융 사기방지 서비스/ 인터넷 뱅킹")
        ...
        if start4:
            ...
            filtered_remote=filtered_remote[['기업이름', '포지션이름', '마감일', '경력_min', '경력_max', '채용형태', 
                 '주요업무',  '자격요건', '우대사항','복지', '기업소개', '표준산업분류','사원 수', '기업 평균 연봉','기업유형',
                    '매출액','하이라이트+태그', '퇴사', '입사', '링크']]
            st.dataframe(data=filtered_remote,hide_index=True)
